...
export const ChatContextProvider = ({ children, user }) => {
    ...
    const memoizedChat = useMemo(() => currentChat, [currentChat?._id]);

    console.log("online users: ", onlineUsers);
    console.log("notification: ", notifications);
    useEffect(() => {
        const newSocket=io("https://chat-app-socket-9nlg.onrender.com/");
        setSocket(newSocket);
    ...
    useEffect(() => {
        const getUsers = async () => {
            if (!user || !user._id) {
                console.log("User is not defined or invalid.");
                return;
            }
        ...
        const getUserChats = async () => {
            if (user && user._id) {
                ...
                try {
                    ...
                    setUserChatsError(error.message || "An unexpected error occurred");
                } finally {
    ...
    useEffect(() => {
        const getMessages = async () => {
            ...
            try {
                const response = await getRequest(`${baseUrl}/api/messages/${memoizedChat?._id}`);
                if (response.error) {
